...
def create_ticket_api():
    ...
    if user.role == 'ad_user':
        ...
        if not comment or (any(c in special_characters for c in comment)):
            return jsonify(
                {"erreur": "le champ comment est obligatoire ou ne doit pas contenir de caractères spéciaux"})
        ...
        return jsonify({"message": "you dot not have the permissions "}), 403
...
def create_token():
    ...
    if role == "ad_user":
        ...
        response = {"token": access_token.decode('utf8'), "user_id": user.id, "username": user.username,
                    "nom": user.nom,
    ...
    asset_list = []
    response = {"token": access_token.decode('utf8'), "user_id": user.id, "username": user.username, "nom": user.nom,
                "prenom": user.prenom, "role": role, "ass": nb_assets, "service": nb_service,
...
def modify_asset_api():
    ...
    if user.role == 'ad_user':
        ...
        return jsonify({"error": "you do not have the permissions"})
...
def delete_product_api():
    ...
    if user.role == 'ad_user':
        ...
        return jsonify({'error': "access denied"}), 201
...
def notification():
    ...
    last_tickets = m.Ticket_notification.filter_by(manager=current_user_id).order_by(
        desc(m.Ticket_notification.created_at))[:5]
