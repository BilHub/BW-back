...
def obso_client():
    ...
    # sélectionner les trois premiers éléments de la liste triée
...
def abonnement_api():
    ...
    if client.subscription:
        ...
        dict_list_sub['cpe_credits'] = abon.cpe_credits
        if abon:
...
def list_vuls_team():
    ...
    if len(tickets) > 0:
        ...
        print("before closed_tickets: !!!!", closed_tickets)
...
def import_assets_api_products():
    current_user_id = get_jwt_identity()
    user = m.User.query.filter_by(id=current_user_id).first_or_404()
    print("calling /import-assets-ap !!!")
    if user.role  == 'ad_user':
      file = request.files['file']
      print("file is: ",file)
      filename = secure_filename(file.filename)
...
def create_cert():
    ...
    if role == 'cert_ad':
        ...
        return jsonify({"msg": "Rôle d'utilisateur invalide"}), 401
...
def dashboard_cert():
    ...
    for inscription in sub:
        mois = dti.datetime.strftime(inscription.start_at,
                                     '%B %Y')  # Extraction du mois à partir de la date d'inscription
    ...
    # Liste des clients avec date de début d'abonnement , date de fin d'abonneemnt et le type d'abonnement
    client = m.Client_group.query.all()
    list_name = []
...
def dashboard_analyst():
    ...
    for count, month in result:
        month_str = month  # Conversion de l'objet datetime en chaîne de caractères
    ...
    ############### nombre de ticket par état ####################
    status_counts = {
        'fermé': m.Ticket.query.filter_by(status=-1).count(),
        'traité': m.Ticket.query.filter_by(status=2).count(),
        'en traitement': m.Ticket.query.filter_by(status=1).count(),
        'en attente': m.Ticket.query.filter_by(status=0).count()
    }
    ################# nombre de vulnérabilité par mois #########################
...
def send_token_to_email_cert():
    ...
    if analyst is None:
        return jsonify({"message": "Aucun analyste avec cette adresse e-mail n'a été trouvé."}), 404
...
def reset_password_cert():
    ...
    if token is None:
        return jsonify({"message": "Le jeton de réinitialisation de mot de passe est invalide ou a expiré."}), 400
    print(token)
...
def delete_user_cert():
    ...
    if user is None:
        ...
        db.session.delete(user)
        db.session.commit()
    return jsonify({"succès": "L'utilisateur a été supprimé avec succès"}), 200
...
def delete_company_cert():
    ...
    if company is None:
        ...
        db.session.commit()
    return jsonify({"succès": "Client a été supprimé avec succès"}), 200
...
def delete_asset_cert(asset_ref):
    ...
    if assets:
        ...
        db.session.commit()
        return jsonify({"succès": "Actif a été supprimé"}), 200
    else:
...
def get_all_pretickets():
    ...
    for pt in pretickets:
        ...
        status_mapping = {0: 'En attente', -1: 'Fermé', 2: 'Traité', 1: 'En traitement'}
...
def get_assets_from_agents():
    get_token_endpoint = "https://51.20.133.51:55000/security/user/authenticate?raw=true"
    all_agents_endpoint = "https://51.20.133.51:55000/agents?pretty=true"
    ...
    if response.status_code == 200:
        ...
        for agent in agents_id_list:
            single_agent_endpoint = f"https://51.20.133.51:55000/syscollector/{agent['id']}/packages?pretty=true&limit=10000"
                ...
                for item in data:
                    ...
                    for element in version_split:
                        try:
                            ...
                            break
                    version = version[:-1]
                    all_data.append({"actif":actif, type:"application", "vendor":vendor, "product":product, "version":version, "manager" : admin_user.username, "responsable" : admin_user.username, "importance":3})
