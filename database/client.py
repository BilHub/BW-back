    ...
    try:
        ...
        for u in usage_list: # list elements {asset_ref: , cpes: {id_cpe1, id_cpe2, id_cpe3, ...}}
            ...
            if asset_id : #  asset already exists in the DB
                ...
                ### extract manager id ####
                select_query = """ select u.id from user u where u.username = %s """
                args = (u['manager'],)
                cursor.execute(select_query, args)
                record = cursor.fetchone()
                id_manager = record['id']
                ### extract responsable id ####
                select_query = """ select u.id from user u where u.username = %s """
                args = (u['responsable'],)
                cursor.execute(select_query, args)
                record = cursor.fetchone()
                id_responsable = record['id']
                ##### extract service id #####
                select_query = """ select u.id from service u where u.name = %s """
                args = (u['service'],)
                cursor.execute(select_query, args)
                record = cursor.fetchone()

                ### verify if the service exists, if not create a new one (added by billel)
                if not record:
                    print("there is a new service in the file !!!!!!")
                    insert_query = """INSERT IGNORE INTO service (name, manager, responsable) 
                                                                    VALUES (%s, %s, %s) """
                    recordTuple = (u['service'], id_manager, id_responsable)
                    cursor.execute(insert_query, recordTuple)
                    connection.commit()
                    select_query = """ select u.id from service u where u.name = %s """
                    args = (u['service'],)
                ...
                assets.append([u['asset_ref'], group_id, 0, 0, u['importance'], id_manager, id_service, id_responsable])  # asset record: [asset_ref,group_id,status,modified,importance,manager,service]
                new_assets.append(u)
            ...
            for na in new_assets:  # list elements {asset_ref: , cpe_list: {id_cpe1, id_cpe2, id_cpe3, ...}}
                ...
                for cpe in na['cpes']:
                    if cpe_credits > 0:
                        ...
                        cpe_credits -= 1
                        #break
        print("asset_usage_records: !!!!", asset_usage_records)
...
""" delete products with agent brightwatch from the database """
def delete_cpes(cpes_list, group_id, actif, connection=None):

    print("cpes_list: ", cpes_list)
    ...
    try:
        ...
        if not connection:
            connection = connect_to_db()
        cursor = connection.cursor()
        query = """delete from asset_usage where asset_ref=%s and cpe=%s"""
        tuple_records = []
        for item in cpes_list:
            tuple_records.append((item["actif"], item["product"]))
